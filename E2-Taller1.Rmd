...
t_ci <- function(x,samples,obs,ci) {
  media=mean(x);  cont=0
  repeat{combined=replicate(samples,sample(x,size=obs))
...
n_ci <- function(x,samples,obs,ci) {
  media=mean(x);  cont=0
  repeat{combined=replicate(samples,sample(x,size=obs))
    if(eq_in_row(combined)==0){break}}
  for (i in 1:samples){
    z=confint(lm(combined[,i]~1),level=ci-0.1)
